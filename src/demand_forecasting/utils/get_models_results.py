...
from .models import get_results_cv_sma, objective_expsmoothing, objective_arima
...
def get_arima_results(global_data: GlobalData, preparated_data_dict: Dict[str, Union[str, pl.DataFrame]],
                             data_separators_dict: Dict[str, Union[List[int], Type[EWS]]], parameters: Dict[str, Any]) -> pl.DataFrame:
    """Get the results of the StatsModels ARIMA model.
    ...
    Returns:
        pl.DataFrame: DataFrame with the results for the StatsModels ARIMA model.
    """

    start = time.time()

    objective = partial(objective_arima, global_data=global_data, preparated_data_dict=preparated_data_dict,
                        data_separators_dict=data_separators_dict, parameters=parameters)
    ...
    return get_results_df(unique_id=unique_id, model="ARIMA", execution_time=execution_time,
                          metrics_values=expsmoothing_metrics, parameters=parameters)